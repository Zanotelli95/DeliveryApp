...
import com.example.delivery.activities.adapters.CategoriesAdapter
...
class RestaurantProductFragment : Fragment() {
    ...
    var productsProviver: ProductsProvider? = null
    ...
    override fun onCreateView(
        ...
        buttonCreate = myView?.findViewById(R.id.buttonRegistro)
        ...
        productsProviver = ProductsProvider(user?.sessionToken!!)
    ...
    private fun createProduct() {
        ...
        if(isValidForm(name, description, priceText)){
            ...
            productsProviver?.create(files, product)?.enqueue(object: Callback<ResponseHttp>{
                override fun onResponse(
    ...
    private fun isValidForm(name: String, description: String, price: String): Boolean{
        ...
        if(description.isNullOrBlank()){
            Toast.makeText(requireContext(), "Ingresa la descripción del producto", Toast.LENGTH_LONG).show()
            return false
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if(requestCode == 101){
                imageFile1 = File(fileUri?.path) //arhivo que se guarda en el servidor
                imageViewProduct1?.setImageURI(fileUri)
            }
            else if(requestCode == 102){
                imageFile2 = File(fileUri?.path) //arhivo que se guarda en el servidor
                imageViewProduct2?.setImageURI(fileUri)

            } else if (requestCode == 103){
                imageFile3 = File(fileUri?.path) //arhivo que se guarda en el servidor
