...
import com.example.delivery.activities.client.address.map.ClienteAddressMapActivity
...
class ClientAddressCreateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editTexNeighborhood = findViewById(R.id.edittext_neighborhood)
    ...
    private fun createAddress(){
        ...
        var neighborhood = editTexNeighborhood?.text.toString()
            ...
            addressProvider?.create(addressModel)?.enqueue(object: Callback<ResponseHttp> {
                override fun onResponse(
                    ...
                    if(response.body() != null){
                        ...
                        Toast.makeText(this@ClientAddressCreateActivity, "Ocurrio un error en la petición", Toast.LENGTH_LONG).show()
                    }
    ...
    private fun isValidForm(address: String, neighborhood: String): Boolean {
        ...
        if (neighborhood.isNullOrBlank()) {
            Toast.makeText(this, "Ingresa la residencia o barrio", Toast.LENGTH_SHORT).show()
            return false
        }
        if(addressLat == 0.0){
    ...
    private fun goToAddressMap() {
        val i = Intent(this, ClienteAddressMapActivity::class.java)
